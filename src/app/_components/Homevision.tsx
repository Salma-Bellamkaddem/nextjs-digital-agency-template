...
// ── Animations (indépendantes du thème) ──
...
// ── Timeline (colonne de gauche) ──
const TIMELINE = [
  {
    icon: TrackChangesIcon,
    text: (
      <>
        Chez Nexsetia, notre vision repose sur une conviction forte :{' '}
        <Box component='span' sx={{ color: BRAND.primary, fontWeight: 700 }}>
          chaque entreprise évolue dans un cadre qui lui est propre
        </Box>
        , un écosystème qui l&apos;influence autant qu&apos;il est influencé par elle.
      </>
    ),
  },
  {
    ...
    text: (
      <>
        C&apos;est pourquoi nous commençons toujours par une{' '}
        <Box component='span' sx={{ color: BRAND.primary, fontWeight: 700 }}>
          analyse approfondie
        </Box>{' '}
        de ce contexte, afin d&apos;en faire la base de chaque action mise en place.
      </>
    ),
  },
  {
    ...
    text: (
      <>
        Nous sommes loin des recettes toutes faites et des modèles copiés-collés :
        une stratégie n&apos;a de valeur que lorsqu&apos;elle est{' '}
        <Box component='span' sx={{ color: BRAND.primary, fontWeight: 700 }}>
          pensée pour la réalité
        </Box>{' '}
        de l&apos;entreprise qu&apos;elle accompagne.
      </>
    ),
  },
  {
    ...
    text: (
      <>
        Car au fond, les solutions standardisées ignorent ce qui fait la{' '}
        <Box component='span' sx={{ color: BRAND.primary, fontWeight: 700 }}>
          singularité
        </Box>{' '}
        d&apos;une entreprise et limitent son{' '}
        <Box component='span' sx={{ color: BRAND.primary, fontWeight: 700 }}>
          potentiel
        </Box>
        .
...
// ── Rangée d'étapes (bas de section) ──
 

// ── Escalier isométrique (illustration de droite) ──
const STAIR_BLOCKS: {
  x: number
  y: number
  w: number
  ...
  { x: 262, y: 448, w: 44, h: 44, label: 'Générer', sub: 'un impact durable' },
  { x: 340, y: 386, w: 46, h: 48, label: 'Agir', sub: 'avec justesse' },
  { x: 418, y: 320, w: 48, h: 52, glow: true, label: 'Analyser', sub: 'les opportunités' },
  { x: 496, y: 248, w: 50, h: 56, glow: true, top: true, label: 'Comprendre', sub: 'le contexte' },
]
...
const LABEL_POS: Record<string, { top: string; left: string }> = {
  Comprendre: { top: '13%', left: '58%' },
  Analyser: { top: '30%', left: '40%' },
  Agir: { top: '58%', left: '78%' },
  Générer: { top: '76%', left: '58%' },
}
      ...
      <Container maxWidth='lg' sx={{ position: 'relative', zIndex: 1 }}>
        <Grid container spacing={{ xs: 6, md: 4 }} alignItems='center'>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            >
              Une vision pensée <br />
              pour <Box component='span' sx={{ color: BRAND.primary }}>votre réalité</Box>
            </Typography>

            {/* Petit connecteur décoratif */}
            <Box
              sx={{
                display: 'flex',
                alignItems: 'center',
            ...
            >
              ...
              <Box
                sx={{
                  height: 1,
                  width: 200,
                  background: `linear-gradient(90deg, ${BRAND.primary}aa, transparent)`,
                  transformOrigin: 'left',
          ...
          {/* ── Colonne illustration escalier ── */}
          <Grid
            size={{ xs: 12, md: 6 }}
            sx={{ display: { xs: 'none', md: 'block' }, animation: `${fadeIn} 1s ease 0.3s both` }}
          >
            <Box sx={{ position: 'relative', width: '100%', pt: '100%' }}>
              <Box sx={{ position: 'absolute', inset: 0 }}>
                ...
                >
                  ...
                  {/* Étincelle lumineuse pulsante sur la marche la plus haute */}
                ...
                {/* Libellés flottants */}
                {STAIR_BLOCKS.filter((b) => b.label).map((b, i) => (
                  <Box
                    key={b.label}
                    sx={{
                      position: 'absolute',
                      top: LABEL_POS[b.label as string].top,
                      left: LABEL_POS[b.label as string].left,
                      px: 2,
                      py: 1.25,
                      borderRadius: 2.5,
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 0.25 }}>
                      ...
                      />
                      <Typography sx={{ fontSize: 12.5, fontWeight: 800, color: TEXT_MAIN, letterSpacing: 0.5, transition: 'color 0.4s ease' }}>
                        {b.label?.toUpperCase()}
                      </Typography>
                    </Box>
                    <Typography sx={{ fontSize: 11.5, color: TEXT_MUTED, pl: 1.5, transition: 'color 0.4s ease' }}>
                      {b.sub}
                    </Typography>
                  </Box>
                ))}
